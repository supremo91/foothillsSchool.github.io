...
function generateAdmissionNumber($con) {
    // Get the current year
    $currentYear = date("Y");

    // Query to retrieve the last auto-incremented admission number from your student table
    $query = "SELECT MAX(Addmission_Number) AS lastAdmissionNumber FROM student";
    ...
    $newAutoIncrementedPart = $autoIncrementedPart + 1;

    // Format the auto-incremented part to have leading zeros (e.g., 001)
    $formattedAutoIncrementedPart = sprintf("%03d", $newAutoIncrementedPart);

    // Combine the auto-incremented part and the current year
    $admissionNumber = $formattedAutoIncrementedPart . '/' . $currentYear;

    return $admissionNumber;
}

$generatedAdmissionNumber = generateAdmissionNumber($con);
?>
                ...
                <div class="card-body">
                    <table class="table table-bordered table-striped" id="myTable">
                        ...
                        <tbody id="student-table-body-placeholder">
                            <?php 
                                $query = "SELECT * FROM student ORDER BY Addmission_Date ASC";
                                $query_run = mysqli_query($con, $query);
                                ...
                                if(mysqli_num_rows($query_run) > 0) {
                                    ...
                                    foreach($query_run as $student) {
                                        ...
                                        // Increment the auto-incremented part
                                        $autoIncrementedPart++;

                                        // Output the admission number for this student
                            ?>
